...
REPORT="${RUNTIME_DIR}/legacy-57991-migration-$(date +%Y%m%d-%H%M%S).report"
...
rollback() {
  local exit_code=$?
  log "migration failed; restoring legacy service on 57991"
  systemctl stop game-video-tool@57991.service || true
  systemctl start game-video-tool.service || true
  curl -fsS --max-time 5 http://127.0.0.1:57991/health >/dev/null || true
...
log "replace legacy service on 57991 with templated service"
systemctl stop game-video-tool.service
...
log "enable templated services and disable legacy boot path"
